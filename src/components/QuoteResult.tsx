      ...
      try {
        const response = await fetch("/api/charteremail", {
          ...
          body: JSON.stringify({
            ...
            message: contactForm.question || quote,
            yachtInterest: type,
          }),
        });
        ...
        // If API route fails, use Formspree as fallback
        console.log("API route not available, falling back to Formspree");
        useFormspree = true;
      }

      // If API route failed or caught an error, use Formspree
      if (!success && useFormspree) {
        // Using Formspree as fallback (free tier)
        const formspreeResponse = await fetch(
          "https://formspree.io/f/mwkgyglj",
          {
            method: "POST",
            headers: {
              "Content-Type": "application/json",
            ...
            body: JSON.stringify({
              ...
              message: contactForm.question || quote,
              yacht: type,
              _subject: `Booking Request: ${type}`,
            }),
          }
        );

        success = formspreeResponse.ok;
      }

      if (success) {
        if (e.target.closest("form")?.className.includes("inquiry")) {
